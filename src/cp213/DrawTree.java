...
public class DrawTree<T extends Comparable<T>> extends JFrame {
	...
	private class DrawPanel extends JComponent {
		...
		private final Insets BORDER_INSETS = new Insets(BORDER_SIZE, BORDER_SIZE, BORDER_SIZE, BORDER_SIZE);
	...
	private static final Font FONT_HEIGHT = new Font("SansSerif", Font.BOLD, 16);
	...
	public DrawTree(final BST<T> tree) {
		...
		this.height = (int) (this.tree.getHeight() * DrawTree.Y_DIFF + DrawTree.Y_DIFF);
	 ...
	 */
	private void drawCenteredString(final Graphics2D g2d, final String text, final Shape shape, final Font font) {
	 ...
	 */
	private void drawNode(final Graphics2D g2d, final TreeNode<T> node, final int splay, final int level) {
		...
		if (node != null) {
			...
			if (node.getLeft() != null) {
				...
				this.drawNode(g2d, node.getLeft(), splay - splayDiff, level + 1);
			}
			if (node.getRight() != null) {
				...
				this.drawNode(g2d, node.getRight(), splay + splayDiff, level + 1);
			}
			// Draw the node circle.
			final Shape circle = new Ellipse2D.Double(x1 - CIRCLE_SIZE / 2, y1 - CIRCLE_SIZE / 2, CIRCLE_SIZE,
					CIRCLE_SIZE);
			...
			this.drawCenteredString(g2d, node.getData().toString(), circle, FONT_VALUE);
			// Draw the node height above the node circle.
			Rectangle2D rect = new Rectangle2D.Double(x1 - CIRCLE_SIZE / 2, y1 - CIRCLE_SIZE, CIRCLE_SIZE,
					CIRCLE_SIZE / 2);
			this.drawCenteredString(g2d, Integer.toString(node.getHeight()), rect, FONT_HEIGHT);
			// Draw the node count below the node circle.
			rect = new Rectangle2D.Double(x1 - CIRCLE_SIZE / 2, y1 + CIRCLE_SIZE / 2, CIRCLE_SIZE, CIRCLE_SIZE / 2);
			this.drawCenteredString(g2d, Integer.toString(node.getCount()), rect, FONT_COUNT);
		}
		return;
	}
	...
	private int getSplayLeft(final TreeNode<T> node, final int level) {
		...
		if (node != null) {
			splay = power(2, this.tree.getHeight() - (level + 1)) + this.getSplayLeft(node.getLeft(), level + 1);
		}
		return splay;
	}
	...
	private int getSplayRight(final TreeNode<T> node, final int level) {
		...
		if (node != null) {
			splay = power(2, this.tree.getHeight() - (level + 1)) + this.getSplayRight(node.getRight(), level + 1);
		}
		return splay;
	}
